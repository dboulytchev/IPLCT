...
Here we use built-in \lama binary operators to specify the semantics of $\oplus$; this approach is good enough for now since owr primary objective is
...
Now we can give a precise answer to the questions asked in section~\ref{intro-semantics}.
The first question was if the expression \lstinline|0*(x/0)| evaluates to zero or crushes. Due to the strictness of our semantics it is undefined in
any state. Indeed
